export class AnimationController {
    constructor(entity) {
        // Referência para a entidade dona desta animação.
        // Normalmente será um GameObject (Player, Ballon, Tree, etc).
        // Isso permite acessar os dados de animação armazenados na entidade.
        this.entity = entity;

        // Nome da animação atual (ex: "idle", "move", "hit").
        this.current = null;

        // Índice do frame atual dentro da animação.
    ...
    // Getter que retorna o "clip" da animação atual.
    ...
    setState(state) {
        // Obtém o objeto de animações da entidade.
        const animation = this.entity?.animation;

        // Se não existir animação ou o objeto estiver vazio,
        // não há nada para atualizar.
        if (!animation || Object.keys(animation).length === 0) return;

        // Resolve qual animação será usada.
        ...
        // Se já estamos na animação correta,
        ...
        // Reinicia o frame da animação.
        this.frame = 0;

        // Reinicia o tempo acumulado da animação.
        this.elapsed = 0;
    }

    update(delta = 1/60) {
        // Obtém o clip da animação atual.
        const clip = this.clip;

        // Se não houver animação definida, não faz nada.
        if (!clip) return;

        // Soma o tempo passado desde o último update.
        this.elapsed += delta;

        // Calcula quanto tempo cada frame deve durar.
        // Exemplo: fps = 8 → cada frame dura 0.125 segundos.
        const frameDuration = 1 / Math.max(1, clip.fps ?? 8);

        // Avança os frames enquanto o tempo acumulado
        ...
        while (this.elapsed >= frameDuration) {
            ...
            // Remove o tempo correspondente a um frame.
            ...
            if (this.frame >= clip.frames.length) {
                ...
                // Se a animação for loopável,
                // reinicia a partir do primeiro frame.
                if (clip.loop) {
                    this.frame = 0;
                } else {

                    // Caso contrário, mantém no último frame.
        ...
        // Sincroniza os dados da animação com a entidade.
    ...
    isFinished() {
        ...
        // Se não existir animação, consideramos que terminou.
        if (!clip) return true;
        ...
        // Verifica se o frame atual já chegou ao último frame.
